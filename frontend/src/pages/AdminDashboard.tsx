...
export const AdminDashboard: React.FC = () => {
  ...
  // CMS Content State
  const initialCMS = getSharedCMS();
  const [cmsBannerText, setCmsBannerText] = useState(initialCMS.bannerText);
  const [cmsBannerEnabled, setCmsBannerEnabled] = useState(initialCMS.bannerEnabled);
  const [cmsSupportPhone, setCmsSupportPhone] = useState(initialCMS.supportPhone);
  const [cmsMandiRates, setCmsMandiRates] = useState(initialCMS.mandiRates);
  const [cmsPublishSuccess, setCmsPublishSuccess] = useState("");

  // Single Source of Truth Master Data States
  ...
  const logAuditAction = (module: any, action: string, recordId: string, prevVal: string, newVal: string) => {
    ...
    saveSharedAuditLogs([newLog as any, ...getSharedAuditLogs()]);
  };
  ...
  const handleToggleFarmerStatus = (farmerId: string) => {
    ...
    if (!f) return;
    const nextStatus: "Active" | "Suspended" = f.accountStatus === "Active" ? "Suspended" : "Active";
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        setFarmers((prev) => {
          const updated = prev.map((item) => (item.id === farmerId ? { ...item, accountStatus: nextStatus } : item));
          saveSharedFarmers(updated as any);
          return updated;
        });
  ...
  const handleDeleteFarmer = (farmerId: string) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        setFarmers((prev) => {
          const updated = prev.filter((item) => item.id !== farmerId);
          saveSharedFarmers(updated as any);
          return updated;
        });
    ...
    setBuyers((prev) => {
      const updated = prev.map((b) => {
        if (b.id === buyerId) {
          ...
          return { ...b, kycStatus: "Verified" as const };
        }
        return b;
      });
      saveSharedBuyers(updated as any);
      return updated;
    });
  };
    ...
    setBuyers((prev) => {
      const updated = prev.map((b) => {
        if (b.id === buyerId) {
          ...
          return { ...b, kycStatus: "Rejected" as const };
        }
        return b;
      });
      saveSharedBuyers(updated as any);
  ...
  const handleToggleBuyerStatus = (buyerId: string) => {
    ...
    if (!b) return;
    const nextStatus: "Active" | "Suspended" = b.accountStatus === "Active" ? "Suspended" : "Active";
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        setBuyers((prev) => {
          const updated = prev.map((item) => (item.id === buyerId ? { ...item, accountStatus: nextStatus } : item));
          saveSharedBuyers(updated as any);
          return updated;
        });
  ...
  const handleDeleteBuyer = (buyerId: string) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        setBuyers((prev) => {
          const updated = prev.filter((item) => item.id !== buyerId);
          saveSharedBuyers(updated as any);
          return updated;
        });
  ...
  const handleDeleteAgreement = (agrId: string) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        setAgreements((prev) => {
          const updated = prev.filter((item) => item.id !== agrId);
          saveSharedAgreements(updated as any);
          return updated;
        });
  ...
  const handleModalFormSubmit = (e: React.FormEvent) => {
    ...
    if (modalType === "farmer") {
      ...
      if (editingItem) {
        setFarmers((prev) => {
          const updated = prev.map((f) => (f.id === editingItem.id ? newFarmer : f));
          saveSharedFarmers(updated as any);
          return updated;
        });
      ...
      if (editingItem) {
        setBuyers((prev) => {
          const updated = prev.map((b) => (b.id === editingItem.id ? newBuyer : b));
          saveSharedBuyers(updated as any);
          return updated;
        });
      ...
      if (editingItem) {
        ...
        logAuditAction("Cold Storage", "Admin Edited Cold Storage Facility", editingItem.id, `Old: ${editingItem.facilityName}`, `New: ${formName} (Place: ${formLocation}, Space: ${capVal} MT)`);
      } else {
        setStorages((prev) => {
        ...
        logAuditAction("Cold Storage", "Admin Added Cold Storage Facility", newId, "None", `Facility: ${formName} at ${formLocation} (${capVal} MT)`);
      }
  ...
  const sidebarNavItems = [
    ...
    { id: "diagnoses", label: "Crop AI Scans", icon: ShieldCheck, badge: diagnoses.length },
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F0] flex font-sans select-none text-gray-900 overflow-hidden">
              ...
              <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4">
                <div className="border-b pb-3">
                  <h4 className="text-base font-extrabold text-gray-900 flex items-center space-x-2">
                    ...
                    <span>Agronomic Crop Recommendation Rules</span>
                  </h4>
                  <p className="text-xs text-gray-500 mt-0.5">NPK fertilizer parameters and target harvest yields for smallholders.</p>
                </div>
            ...
            <div className="space-y-6 bg-white p-6 rounded-3xl border border-emerald-100 shadow-sm">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b pb-4">
                ...
                <button
                  onClick={() => {
                    saveSharedCMS({
                      bannerText: cmsBannerText,
                      bannerEnabled: cmsBannerEnabled,
                      supportPhone: cmsSupportPhone,
                      noticeText: cmsBannerText,
                      mandiRates: cmsMandiRates
                    });
              ...
              <div className="bg-white p-6 rounded-3xl border border-emerald-100 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium mt-0.5">
                      Live neural network leaf disease scans submitted by farmers. Review images, verify diagnoses, and provide advisory notes.
                    </p>
                  </div>
      ...
      {/* 2.5 CROP DIAGNOSIS FULL INSPECTION MODAL */}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => {
                    ...
                    setDiagnoses(getSharedDiagnoses());
                    logAuditAction("Diagnosis", `Agronomist Verified ${selectedDiagnosisPreview.crop} Pathology`, selectedDiagnosisPreview.id, "Status: New", `Verified (${note})`);
                    setSelectedDiagnosisPreview(null);
                  }}
                  className="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold text-xs rounded-xl shadow-md cursor-pointer transition"
                >
                  ✓ Mark Agronomist Verified & Send Advisory
                </button>
              </div>
            </div>
          </div>
